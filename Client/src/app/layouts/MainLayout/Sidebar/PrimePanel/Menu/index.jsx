...
export function Menu({ nav, pathname }) {
  ...
  // PrimePanel shows child items - no filtering needed as parent already filtered
  const filteredNav = useMemo(() => {
    return nav;
  }, [nav]);
  const initialActivePath = useMemo(() => {
    return filteredNav.find((item) => isRouteActive(item.path, pathname))?.path;
  ...
  useDidUpdate(recalculate, [filteredNav]);
